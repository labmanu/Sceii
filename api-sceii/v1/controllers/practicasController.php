...
include_once('../modelsDAO/practicasDAO.php');
include_once('../authorization/authorization.php');
include_once('../models/usuario.php');
require_once('responseHttp.php');

    class practicasController extends responseHttp{

      
        function crea_practica($data){
            try{
            $auth = new authorization();
            $token_data = $auth->authorizationByTypeUSer("jefe laboratorio");
            //echo ($token_data['data']['id']);
            $practicas = new practicasDAO();
           $status = $practicas->crear($data['nombre'], $data['id_laboratorio'], $token_data['data']['id']);
